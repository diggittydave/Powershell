...
$maxThreads = 20
$file = "C:\Users\chq-davidwe\Documents\Projects\web-stuff\results\results-ETMS.txt"
$results = "C:\Users\chq-davidwe\Documents\Projects\web-stuff\results\results-ETMS.csv"
...
Function Stream-jobs{
    Foreach($element in $ARRAY1){
        ...
        Write-Host "$bbb"
        Start-Job -ScriptBlock { . 'C:\Users\chq-davidwe\Documents\Projects\web-stuff\ETMS\get-stuff-ETMS.ps1' -bbb $args[0]} -ArgumentList "$bbb"
        }
...
Function Receive-jobs{
    Foreach($element in $ARRAY1){
        $bbb = "$element"
        while((Get-Job -State Running).count -ge $maxThreads){start-sleep 1}
        Start-Job -ScriptBlock { . 'C:\Users\chq-davidwe\Documents\Projects\web-stuff\ETMS\parse-stuff-ETMS.ps1' -bbb $args[0]} -ArgumentList "$bbb"
    }
}
