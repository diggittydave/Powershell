...
$maxThreads = 20
$file = "C:\Users\chq-davidwe\Documents\Projects\web-stuff\results\results-INVPROC.txt"
$results = "C:\Users\chq-davidwe\Documents\Projects\web-stuff\results\results-INVPROC.csv"
...
Function Stream-jobs{
    Foreach($element in $ARRAY1){
        ...
        Write-Host "$bbb"
        Start-Job -ScriptBlock { . 'C:\Users\chq-davidwe\Documents\Projects\web-stuff\INVPROC\get-stuff-INVPROC.ps1' -bbb $args[0]} -ArgumentList "$bbb"
        }
...
Function Receive-jobs{
    Foreach($element in $ARRAY1){
        $bbb = "$element"
        while((Get-Job -State Running).count -ge $maxThreads){start-sleep 1}
        Start-Job -ScriptBlock { . 'C:\Users\chq-davidwe\Documents\Projects\web-stuff\INVPROC\parse-stuff-INVPROC.ps1' -bbb $args[0]} -ArgumentList "$bbb"
    }
}
